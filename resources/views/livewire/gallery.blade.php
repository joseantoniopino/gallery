<div>
    @if($show === true)
        <div wire:click="close()" class="absolute top-0 left-0 w-full h-full overflow-y-hidden bg-custom-950/80 backdrop-blur-md"></div>

        <div class="fixed flex flex-row justify-between inset-x-0 mx-auto w-[1248px] top-20 h-12 z-20">
            <p class="text-white text-xl leading-[23.48px] font-medium">Image Gallery for {{ $galleryModel->getImageableName() }} <strong>{{ $galleryModel->getName() }}</strong></p>
        ...
        <div class="fixed inset-x-0 mx-auto inset-y-0 my-auto w-[1248px] h-[720px] z-20 rounded-lg bg-white/20">
            <div
                ...
                @foreach($galleryModel->images as $image)
                    <div wire:key="{{ $image->id }}" class="relative w-[336px] h-[300px] rounded-lg">
                        <div
                            wire:click="toggleFavorite({{ $image }})"
                            class="absolute flex justify-center items-stretch top-[19px] right-[19.82px] w-[41.63px] h-[43px] rounded-full z-40 cursor-pointer @if($image->is_favorite) bg-[#3F54D1] hover:brightness-200 @else bg-black/[0.15] hover:backdrop-brightness-200 @endif"
                        ...
                        <img class="absolute top-0 left-0 w-full h-full rounded-lg z-30" src="{{ asset($image->path) }}"
                             alt="{{ $image->alt }}">
